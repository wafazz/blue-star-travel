...
<title>Blue Travel — Sign In</title>
...
<div class="app">
  ...
  <div class="hero">
    ...
    <div class="badge">🔒 Secure Customer Sign In</div>
  </div>

  <div class="card">
    <h2>Welcome back 👋</h2>
    <div class="sub">Sign in to manage your bookings &amp; trips</div>

    @if ($errors->any())
      <div class="err">⚠️ <span>{{ $errors->first() }}</span></div>
    @endif

    <form id="loginForm" method="POST" action="{{ route('login') }}" onsubmit="return doLogin()">
      @csrf
      <div class="field">
        <label>Email</label>
        <div class="inp">
      ...
      <div class="row">
        ...
        <a class="forgot" onclick="toast('Password reset link sent 📧')">Forgot password?</a>
      </div>
  ...
  <div class="hintbox" data-ios-install hidden style="display:flex;align-items:center;justify-content:center;gap:6px;flex-wrap:wrap;margin-top:2px">
    <span>Install Blue Star: tap</span>
